...
![[Pasted image 20220921223619.png]]
created: 网络请求, 初始化数据, 不能获取dom
attached: 获取 dom
dettached: 回收
```js
...
pageLifetimes
show/hide/resize
![[Pasted image 20220921223645.png]]
...
## 4.1 properties  定义传入的属性
![[Pasted image 20220921224629.png]]
## 4.2 data  定义内部属性
![[Pasted image 20220921224640.png]] ## methods  定义方法
![[Pasted image 20220921224653.png]]
## 4.3 options  额外配置选项 
![[Pasted image 20220921224703.png]]
## 4.4 externalClasses  引用外部样式
![[Pasted image 20220921224746.png]]
## 4.5 observes  属性和数据监听 
![[Pasted image 20220921224735.png]]
## 4.6 pageLifetimes  页面生命周期 
![[Pasted image 20220921224725.png]]
## 4.7 lefetimes  组件生命周期 
![[Pasted image 20220921224716.png]]
...
```html
<!--pages/gg/gg.wxml-->
<text>pages/gg/gg.wxml</text>
<view class="tab">
  <block wx:for="{{arr}}" wx:key="*this">
    <view class="block {{index === currentIndex ? 'active': ''}}" bindtap="itemTap" data-index="{{index}}">
...
子
```html
<!--components/f-tab/f-tab.wxml-->
<text>components/f-tab/f-tab.wxml</text>
<view class="tab">
  <block wx:for="{{arr}}" wx:key="*this">
    <view class="block {{index === currentIndex ? 'active': ''}}" bindtap="itemTap" data-index="{{index}}">
    <text class="item">{{item}}</text>
